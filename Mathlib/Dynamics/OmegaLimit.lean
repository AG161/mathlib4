...
theorem omegaLimit_union : ω f ϕ (s₁ ∪ s₂) = ω f ϕ s₁ ∪ ω f ϕ s₂ := by
  ...
  exact Subset.trans <| inter_subset_inter_right _ <| preimage_mono <| inter_subset_left _ _
  exact Subset.trans <| inter_subset_inter_right _ <| preimage_mono <| inter_subset_right _ _
  rintro (hy | hy)
